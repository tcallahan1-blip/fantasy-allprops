#!/usr/bin/env node
/**
 * Fantasy AllProps — generate-enrichments.js
 *
 * Reads data/season-1-events.json and writes supabase/enrichments.sql
 * containing UPDATE statements for every event with description, website_url,
 * and options (array of valid pick strings, or null where free-text is better).
 *
 * Usage:  node scripts/generate-enrichments.js
 */
...
/** Render an options array (or null) as SQL fragment */
function renderOptions(opts) {
  if (opts === null || opts === undefined) {
    return 'NULL';
  }
  return `'${sqlEsc(JSON.stringify(opts))}'::jsonb`;
}

// ---------------------------------------------------------------------------
// Data sets (current as of 2026-06-10)
// ---------------------------------------------------------------------------
...
// 2025-26 PL teams (for the 2026-27 season the field will be the 25-26 survivors + promoted teams)
// Using the same 20 clubs since the 2026-27 promoted three are unknown; placeholder set is the 2025-26 side.
const PREMIER_LEAGUE_2026_27 = [
  'Arsenal', 'Aston Villa', 'Bournemouth', 'Brentford', 'Brighton & Hove Albion',
...
const TOP_GOLFERS = [
  ...
  'Viktor Hovland', 'Ludvig Åberg', 'Tommy Fleetwood', 'Jon Rahm',
  'Patrick Cantlay', 'Wyndham Clark', 'Brian Harman', 'Max Homa',
  'Hideki Matsuyama', 'Shane Lowry', 'Justin Thomas', 'Jordan Spieth',
  'Brooks Koepka', 'Dustin Johnson', 'Cameron Smith', 'Tony Finau',
  'Adam Scott', 'Corey Conners', 'Jason Day', 'Min Woo Lee',
  'Tom Kim', 'Keegan Bradley', 'Nick Taylor', 'Sungjae Im',
  'Seamus Power', 'Matt Fitzpatrick', 'Tyrrell Hatton', 'Russell Henley',
  'Nick Dunlap', 'Akshay Bhatia', 'Jake Knapp', 'Harris English',
  'Davis Thompson', 'Byeong Hun An', 'Stephan Jaeger', 'Si Woo Kim',
  'Chris Kirk', 'Eric Cole', 'Ben Griffin', 'Joe Highsmith',
  'Nicolai Højgaard', 'Rasmus Højgaard', 'Ryan Fox', 'Thomas Detry',
  'Sepp Straka', 'Thriston Lawrence', 'Francesco Molinari', 'Sergio Garcia',
  'Phil Mickelson', 'Bubba Watson', 'Will Zalatoris', 'Rickie Fowler',
  'Cameron Young', 'Adam Hadwin', 'Kurt Kitayama', 'JT Poston',
];

const TOP_ATP_PLAYERS = [
  'Jannik Sinner', 'Carlos Alcaraz', 'Novak Djokovic', 'Alexander Zverev',
  'Daniil Medvedev', 'Casper Ruud', 'Andrey Rublev', 'Grigor Dimitrov',
  'Hubert Hurkacz', 'Alex de Minaur', 'Taylor Fritz', 'Ben Shelton',
  'Frances Tiafoe', 'Tommy Paul', 'Sebastian Korda', 'Ugo Humbert',
  'Holger Rune', 'Arthur Fils', 'Lorenzo Musetti', 'Stefanos Tsitsipas',
  'Jack Draper', 'Felix Auger-Aliassime', 'Tallon Griekspoor', 'Alejandro Davidovich Fokina',
  'Sebastian Baez', 'Jiri Lehecka', 'Flavio Cobolli', 'Alexei Popyrin',
  'Matteo Berrettini', 'Francisco Cerundolo', 'Tomas Martin Etcheverry', 'Jan-Lennard Struff',
  'Lorenzo Sonego', 'Mariano Navone', 'Nuno Borges', 'Luciano Darderi',
  'Luca Van Assche', 'Jakub Mensik', 'Giovanni Mpetshi Perricard', 'Miomir Kecmanovic',
];

const TOP_WTA_PLAYERS = [
  'Aryna Sabalenka', 'Iga Swiatek', 'Coco Gauff', 'Elena Rybakina',
  'Jasmine Paolini', 'Zheng Qinwen', 'Mirra Andreeva', 'Madison Keys',
  'Emma Navarro', 'Daria Kasatkina', 'Paula Badosa', 'Barbora Krejcikova',
  'Anna Kalinskaya', 'Donna Vekic', 'Linda Noskova', 'Beatriz Haddad Maia',
  'Maria Sakkari', 'Caroline Wozniacki', 'Elina Svitolina', 'Victoria Azarenka',
  'Jessica Pegula', 'Amanda Anisimova', 'Marketa Vondrousova', 'Sofia Kenin',
  'Clara Tauson', 'Diana Shnaider', 'Yulia Putintseva', 'Liudmila Samsonova',
  'Magda Linette', 'Katerina Siniakova', 'Varvara Gracheva', 'Ekaterina Alexandrova',
  'Marie Bouzkova', 'Elise Mertens', 'Harriet Dart', 'Katie Boulter',
  'Naomi Osaka', 'Sloane Stephens', 'Caroline Garcia', 'Alycia Parks',
];
...
const F1_DRIVERS_2026 = [
  ...
  'Nico Hulkenberg (Sauber/Audi)', 'Gabriel Bortoleto (Sauber/Audi)',
  'Yuki Tsunoda (Racing Bulls)', 'Isack Hadjar (Racing Bulls)',
  'Alexander Albon (Williams)', 'Carlos Sainz (Williams)',
];

const TDF_GC_CONTENDERS = [
  'Jonas Vingegaard', 'Tadej Pogacar', 'Primoz Roglic', 'Remco Evenepoel',
  'Carlos Rodriguez', 'Adam Yates', 'Simon Yates', 'Egan Bernal',
  'David Gaudu', 'Enric Mas', 'Wilco Kelderman', 'Tom Pidcock',
  'Mattias Skjelmose', 'Ben O\'Connor', 'Felix Gall', 'Thibaut Pinot',
  'Mikel Landa', 'Alejandro Valverde', 'Marc Soler', 'Guillaume Martin',
];

const TDF_FEMMES_CONTENDERS = [
  'Demi Vollering', 'Annemiek van Vleuten', 'Elisa Longo Borghini',
  'Cecilie Uttrup Ludwig', 'Ashleigh Moolman-Pasio', 'Juliette Labous',
  'Lotte Kopecky', 'Kasia Niewiadoma', 'Niamh Fisher-Black', 'Puck Pieterse',
  'Grace Brown', 'Pauliena Rooijakkers', 'Marlen Reusser', 'Kristen Faulkner',
  'Silvia Persico',
];
...
const MLS_TEAMS = [
  ...
  'Chicago Fire FC', 'FC Cincinnati', 'Colorado Rapids', 'Columbus Crew',
  'D.C. United', 'FC Dallas', 'Houston Dynamo FC', 'Inter Miami CF',
...
const UCL_CONTENDERS = [
  'Real Madrid', 'Manchester City', 'Bayern Munich', 'Liverpool',
  'Arsenal', 'Chelsea', 'Barcelona', 'Atletico Madrid',
  'Inter Milan', 'AC Milan', 'Napoli', 'Juventus',
  'Paris Saint-Germain', 'Borussia Dortmund', 'RB Leipzig', 'Bayer Leverkusen',
  'Ajax', 'Porto', 'Benfica', 'Sporting CP',
  'Feyenoord', 'PSV Eindhoven', 'Shakhtar Donetsk', 'Dinamo Zagreb',
  'Celtic', 'Rangers', 'Red Bull Salzburg', 'Galatasaray',
  'Fenerbahce', 'Lazio', 'Villarreal', 'Sevilla',
];

const BALLON_DOR_CONTENDERS = [
  'Erling Haaland', 'Kylian Mbappe', 'Vinicius Jr', 'Jude Bellingham',
  'Rodri', 'Lamine Yamal', 'Phil Foden', 'Florian Wirtz',
  'Federico Valverde', 'Ruben Dias', 'Bukayo Saka', 'Declan Rice',
  'Harry Kane', 'Robert Lewandowski', 'Neymar Jr', 'Raphinha',
  'Gavi', 'Pedri', 'Dani Olmo', 'Toni Kross',
  'Virgil van Dijk', 'Mohamed Salah', 'Trent Alexander-Arnold', 'Kevin De Bruyne',
  'Marcus Rashford', 'Riyad Mahrez', 'Lionel Messi', 'Cristiano Ronaldo',
  'Alexia Putellas', 'Aitana Bonmati',
];

// Esports teams
const DOTA2_TI_TEAMS = [
  'Team Liquid', 'OG', 'Evil Geniuses', 'Team Secret',
  'Gaimin Gladiators', 'BetBoom Team', 'Tundra Esports', 'Cloud9',
  'Natus Vincere', 'Virtus.pro', '9Pandas', 'Team Falcons',
  'Talon Esports', 'Xtreme Gaming', 'Azure Ray', 'Spirit',
];

const LOL_WORLDS_TEAMS = [
  'T1', 'Gen.G', 'JDG Intel Esports Club', 'BiliBili Gaming',
  'Weibo Gaming', 'LNG Esports', 'Top Esports', 'NRG Esports',
  'Cloud9', 'Team Liquid', 'Fnatic', 'G2 Esports',
  'KT Rolster', 'DRX', 'Team WE', '100 Thieves',
];

const VALORANT_CHAMPIONS_TEAMS = [
  'Sentinels', 'NRG Esports', 'Cloud9', '100 Thieves',
  'Loud', 'Fnatic', 'Team Liquid', 'G2 Esports',
  'Paper Rex', 'Natus Vincere', 'Team Vitality', 'Evil Geniuses',
  'Trace Esports', 'Bilibili Gaming', 'GenG Esports', 'Giants Gaming',
];

const CS2_IEM_COLOGNE_TEAMS = [
  'Natus Vincere', 'FaZe Clan', 'Vitality', 'G2 Esports',
  'Team Liquid', 'Cloud9', 'MOUZ', 'Heroic',
  'FURIA Esports', 'BIG', 'Astralis', 'NIP',
  'Virtus.pro', 'OG', 'Monte', 'Complexity Gaming',
];

const RUGBY_LEAGUE_WC_TEAMS = [
  'Australia', 'New Zealand', 'England', 'Papua New Guinea',
  'Samoa', 'Tonga', 'France', 'Fiji',
  'Polynesia', 'Jamaica', 'Lebanon', 'Scotland',
  'Ireland', 'Wales', 'Italy', 'Cook Islands',
];

const UEFA_WOMEN_CL_CONTENDERS = [
  'Barcelona', 'Chelsea', 'Lyon', 'Bayern Munich',
  'Arsenal', 'Manchester City', 'Real Madrid', 'Paris Saint-Germain',
  'Wolfsburg', 'Roma', 'Ajax', 'Brann',
  'Manchester United', 'Atletico Madrid', 'Juventus', 'Benfica',
];

const FIBA_WOMEN_NATIONS = [
  'United States', 'Australia', 'Canada', 'France',
  'Spain', 'Belgium', 'China', 'Germany',
  'Japan', 'South Korea', 'Great Britain', 'Nigeria',
  'Brazil', 'Serbia', 'Puerto Rico', 'Mali',
];

const CONFEDERATIONS_CUP_NATIONS = [
  'Brazil', 'Argentina', 'France', 'Germany',
  'Spain', 'England', 'Portugal', 'Italy',
  'Netherlands', 'Belgium', 'United States', 'Mexico',
  'Japan', 'South Korea', 'Australia', 'Morocco',
  'Senegal', 'Egypt', 'Colombia', 'Uruguay',
];
...
const NASCAR_DRIVERS = [
  ...
  'Martin Truex Jr', 'Christopher Bell', 'Denny Hamlin', 'Joey Logano',
  'Ryan Blaney', 'Brad Keselowski', 'Tyler Reddick', 'Austin Cindric',
  'Bubba Wallace', 'Erik Jones', 'Michael McDowell', 'Todd Gilliland',
  'Kevin Harvick', 'Aric Almirola', 'Corey LaJoie', 'Ricky Stenhouse Jr',
];

const INDY_DRIVERS = [
  'Scott Dixon', 'Will Power', 'Pato O\'Ward', 'Josef Newgarden',
  'Marcus Ericsson', 'Simon Pagenaud', 'Graham Rahal', 'Colton Herta',
  'Alexander Rossi', 'Rinus VeeKay', 'Felix Rosenqvist', 'Christian Lundgaard',
  'Helio Castroneves', 'Tony Kanaan', 'Ed Carpenter', 'Conor Daly',
];

const SNOOKER_PLAYERS = [
  'Ronnie O\'Sullivan', 'Mark Selby', 'Judd Trump', 'Neil Robertson',
  'Mark Williams', 'John Higgins', 'Barry Hawkins', 'Kyren Wilson',
  'Ding Junhui', 'Luca Brecel', 'Si Jiahui', 'Zhang Anda',
  'Mark Allen', 'Stephen Maguire', 'Ryan Day', 'Jack Lisowski',
  'Stuart Bingham', 'Tom Ford', 'Gary Wilson', 'Shaun Murphy',
];

const PDC_DARTS_PLAYERS = [
  'Luke Littler', 'Michael van Gerwen', 'Luke Humphries', 'Gerwyn Price',
  'Peter Wright', 'Dimitri Van den Bergh', 'Michael Smith', 'Gary Anderson',
  'Rob Cross', 'Joe Cullen', 'Nathan Aspinall', 'Jonny Clayton',
  'Chris Dobey', 'Danny Noppert', 'Ryan Searle', 'Andrew Gilding',
  'Mike de Decker', 'Josh Rock', 'Callan Rydz', 'Dave Chisnall',
];

const SURFING_ELITE = [
  'Griffin Colapinto', 'Ethan Ewing', 'Jack Robinson', 'Filipe Toledo',
  'John John Florence', 'Italo Ferreira', 'Gabriel Medina', 'Kanoa Igarashi',
  'Samuel Pupo', 'Ryan Callinan', 'Maxime Huscenot', 'Morgan Cibilic',
  'Molly Picklum', 'Caitlin Simmers', 'Caroline Marks', 'Tatiana Weston-Webb',
  'Johanne Defay', 'Bettylou Sakura Johnson', 'Isabella Nichols', 'Teresa Bonvalot',
];

const IRONMAN_CONTENDERS = [
  'Patrick Lange', 'Jan Frodeno', 'Sam Laidlow', 'Kristian Blummenfelt',
  'Alistair Brownlee', 'Gustav Iden', 'Sebastian Kienle', 'Tim O\'Donnell',
  'Lionel Sanders', 'Denis Chevrot',
  // Women
  'Lucy Charles-Barclay', 'Chelsea Sodaro', 'Anne Haug', 'Laura Philipp',
  'Kat Matthews', 'Paula Findlay', 'Sarah Crowley', 'Daniela Ryf',
  'Skye Moench', 'Fenella Langridge',
];

const CYCLING_GC_WOMEN = [
  'Demi Vollering', 'Annemiek van Vleuten', 'Elisa Longo Borghini',
  'Cecilie Uttrup Ludwig', 'Lotte Kopecky', 'Kasia Niewiadoma',
  'Niamh Fisher-Black', 'Puck Pieterse', 'Grace Brown', 'Pauliena Rooijakkers',
  'Marlen Reusser', 'Silvia Persico', 'Sofia Bertizzolo', 'Juliette Labous',
  'Elisa Chabbey',
];

const CHESS_PLAYERS = [
  'Magnus Carlsen', 'Fabiano Caruana', 'Hikaru Nakamura', 'Alireza Firouzja',
  'Ian Nepomniachtchi', 'Anish Giri', 'Levon Aronian', 'Viswanathan Anand',
  'Richard Rapport', 'Wesley So', 'Shakhriyar Mamedyarov', 'Teimour Radjabov',
  'Dmitry Andreikin', 'Vincent Keymer', 'Nodirbek Abdusattorov',
];

const WRESTLING_CHAMPIONS = [
  'Cody Rhodes', 'Roman Reigns', 'CM Punk', 'Seth Rollins',
  'Gunther', 'Sami Zayn', 'Kevin Owens', 'Drew McIntyre',
  'Brock Lesnar', 'LA Knight', 'Rhea Ripley', 'Becky Lynch',
  'Charlotte Flair', 'Bianca Belair', 'Bayley', 'Naomi',
];

const FIFA_CONFEDERATIONS_NATIONS = [
  'Brazil', 'Argentina', 'France', 'Spain',
  'England', 'Germany', 'Portugal', 'Italy',
  'Netherlands', 'United States', 'Mexico', 'Japan',
  'South Korea', 'Morocco', 'Senegal', 'Australia',
];

const ESPORTS_WORLD_CUP_GAMES = [
  'League of Legends', 'Dota 2', 'CS2', 'Valorant',
  'Rocket League', 'Free Fire', 'PUBG Mobile', 'EA Sports FC',
  'Street Fighter 6', 'Fortnite',
];

// ---------------------------------------------------------------------------
// Enrichment definitions — keyed by event slug
// ---------------------------------------------------------------------------
...
const enrichments = {
  ...
  'aug-2026-commonwealth': {
    ...
    options: ['Australia', 'England', 'Canada', 'India', 'South Africa', 'New Zealand', 'Scotland', 'Jamaica', 'Wales', 'Nigeria'],
  },
  ...
  'aug-2026-tdf-femmes': {
    ...
    options: TDF_FEMMES_CONTENDERS,
  },

  'aug-2026-european-athletics': {
    ...
    options: ['Great Britain', 'France', 'Germany', 'Netherlands', 'Poland', 'Italy', 'Spain', 'Norway', 'Sweden', 'Portugal', 'Ukraine', 'Czech Republic', 'Belgium', 'Hungary', 'Finland'],
  },
  ...
  'aug-2026-ti15': {
    ...
    options: DOTA2_TI_TEAMS,
  },

  'aug-2026-us-open-tennis-start': {
    ...
    options: [...TOP_ATP_PLAYERS, ...TOP_WTA_PLAYERS],
  },
  ...
  'aug-2026-pl-opens': {
    ...
    options: PREMIER_LEAGUE_2026_27,
  },
  ...
  'sep-2026-fiba-women': {
    ...
    options: FIBA_WOMEN_NATIONS,
  },

  'sep-2026-valorant-champions': {
    ...
    options: VALORANT_CHAMPIONS_TEAMS,
  },

  'sep-2026-nfl-kickoff': {
  ...
  'sep-2026-ucl-groups': {
    ...
    options: UCL_CONTENDERS,
  },

  'sep-2026-ncaa-football': {
    ...
    options: [
      ...
      'Clemson', 'Oklahoma', 'USC', 'Oregon', 'Penn State', 'Tennessee',
      'Florida State', 'LSU', 'Miami', 'Colorado', 'Washington', 'Utah',
      'TCU', 'Kansas State', 'UCLA', 'Ole Miss', 'Iowa', 'Wisconsin',
    ],
  },

  'sep-2026-sweden-election': {
    ...
    options: ['Sweden Democrats / centre-right bloc wins majority', 'Social Democrats / centre-left bloc wins majority', 'Hung parliament / no clear majority'],
  },

  'sep-2026-booker-shortlist': {
  ...
  'oct-2026-lol-worlds': {
    ...
    options: LOL_WORLDS_TEAMS,
  },

  'oct-2026-brazil-round1': {
    ...
    options: ['Luiz Inácio Lula da Silva (PT)', 'Jair Bolsonaro (PL)', 'Other / Third-party candidate'],
  },

  'oct-2026-quebec-election': {
    ...
    options: ['Coalition Avenir Québec (CAQ)', 'Parti Québécois (PQ)', 'Québec solidaire (QS)', 'Quebec Liberal Party (PLQ)', 'Conservative Party of Quebec'],
  },
  ...
  'oct-2026-ironman': {
    ...
    options: IRONMAN_CONTENDERS,
  },

  'oct-2026-chicago-marathon': {
  ...
  'nov-2026-midterms': {
    ...
    options: ['Democrats win House, Democrats win Senate', 'Republicans win House, Republicans win Senate', 'Democrats win House, Republicans win Senate', 'Republicans win House, Democrats win Senate'],
  },

  'nov-2026-breeders-cup': {
  ...
  'nov-2026-atp-finals': {
    ...
    options: TOP_ATP_PLAYERS,
  },

  'nov-2026-dwts-finale': {
  ...
  'nov-2026-f1-sao-paulo': {
    ...
    options: F1_DRIVERS_2026,
  },

  'nov-2026-f1-las-vegas': {
  ...
  'dec-2026-wsl-pipeline': {
    ...
    options: SURFING_ELITE,
  },

  'dec-jan-pdc-darts-start': {
  ...
  'dec-2026-euro-handball': {
    ...
    options: ['Norway', 'Denmark', 'France', 'Netherlands', 'Sweden', 'Germany', 'Spain', 'Hungary', 'Romania', 'Croatia', 'Montenegro', 'Slovenia', 'Serbia', 'Czech Republic', 'Poland', 'Austria'],
  },

  'dec-2026-si-sportsperson': {
  ...
  'jan-2027-united-cup': {
    ...
    options: ['Australia', 'United States', 'Spain', 'Italy', 'Great Britain', 'Poland', 'Germany', 'Czech Republic', 'Argentina', 'France', 'Greece', 'Canada'],
  },

  'jan-2027-nhl-winter-classic': {
  ...
  'feb-2027-westminster': {
    ...
    options: ['Herding Group', 'Hound Group', 'Non-Sporting Group', 'Sporting Group', 'Terrier Group', 'Toy Group', 'Working Group'],
  },

  'feb-2027-nfl-combine': {
  ...
  'feb-2027-daytona-500': {
    ...
    options: NASCAR_DRIVERS,
  },

  'feb-2027-nba-allstar': {
    ...
    options: ['Eastern Conference All-Stars', 'Western Conference All-Stars'],
  },

  'feb-2027-traitors-us': {
  ...
  'mar-2027-ncaa-mens': {
    ...
    options: [
      'Duke', 'Kentucky', 'Kansas', 'North Carolina', 'Gonzaga', 'Villanova',
      'Michigan State', 'Indiana', 'Louisville', 'Connecticut', 'Syracuse',
      'Florida', 'Michigan', 'Ohio State', 'Virginia', 'Arizona',
      'Texas', 'UCLA', 'Baylor', 'Houston', 'Purdue', 'Tennessee',
      'Alabama', 'Marquette', 'Creighton', 'Iowa State', 'Arkansas',
    ],
  },

  'mar-2027-ncaa-womens': {
    ...
    website_url: 'https://www.ncaa.com/march-madness',
    options: [
      'South Carolina', 'Iowa', 'LSU', 'Connecticut', 'Notre Dame',
      'Stanford', 'Texas', 'Tennessee', 'Indiana', 'North Carolina State',
      'Virginia Tech', 'Baylor', 'Kansas State', 'Colorado', 'Oregon',
      'Maryland', 'Ohio State', 'Michigan', 'Villanova', 'Gonzaga',
    ],
  },

  'mar-2027-wrestlemania': {
  ...
  'mar-2027-six-nations': {
    ...
    options: SIX_NATIONS_TEAMS,
  },

  'mar-2027-indian-wells': {
  ...
  'mar-2027-crufts': {
    ...
    options: ['Afghan Hound', 'Border Collie', 'German Shepherd', 'Golden Retriever', 'Labrador Retriever', 'Miniature Poodle', 'Whippet', 'Flat-Coated Retriever', 'Welsh Terrier', 'Cocker Spaniel', 'Borzoi', 'Irish Setter'],
  },
  ...
  'mar-2027-chess': {
    ...
    options: CHESS_PLAYERS,
  },

  'mar-2027-players-championship': {
  ...
  'apr-2027-ncaa-mens-final': {
    ...
    options: [
      'Duke', 'Kentucky', 'Kansas', 'North Carolina', 'Gonzaga', 'Villanova',
      'Michigan State', 'Connecticut', 'Baylor', 'Houston', 'Purdue',
      'Tennessee', 'Alabama', 'Iowa State', 'Arkansas', 'Creighton',
    ],
  },

  'apr-2027-ncaa-womens-final': {
    ...
    options: [
      'South Carolina', 'Iowa', 'LSU', 'Connecticut', 'Notre Dame',
      'Stanford', 'Texas', 'Tennessee', 'Indiana', 'North Carolina State',
      'Baylor', 'Kansas State', 'Oregon', 'Maryland', 'Ohio State',
    ],
  },

  'apr-2027-voice-finale': {
  ...
  'apr-2027-sidemen': {
    ...
    options: ['Sidemen FC', 'YouTubers XI'],
  },

  'apr-2027-world-snooker': {
  ...
  'may-2027-uk-local-elections': {
    ...
    options: ['Labour gains most councils', 'Conservative gains most councils', 'Liberal Democrats gain most councils', 'Reform UK gains most councils', 'No clear national swing'],
  },

  'may-2027-cheese-rolling': {
  ...
  'may-2027-indy-500': {
    ...
    options: INDY_DRIVERS,
  },

  'may-2027-spelling-bee': {
  ...
  'may-2027-conference-league': {
    ...
    options: [...UCL_CONTENDERS, 'Fiorentina', 'Anderlecht', 'PAOK', 'Slavia Prague', 'Rapid Vienna'],
  },
  ...
  'may-2027-uwcl': {
    ...
    options: UEFA_WOMEN_CL_CONTENDERS,
  },

  'may-2027-natsu-basho': {
  ...
  'jun-2027-nba-awards': {
    ...
    options: [...NBA_TEAMS, ...Array.from(new Set([
      'Shai Gilgeous-Alexander', 'Victor Wembanyama', 'Jayson Tatum', 'Luka Doncic',
      'Anthony Edwards', 'Giannis Antetokounmpo', 'Joel Embiid', 'Nikola Jokic',
      'Donovan Mitchell', 'Tyrese Haliburton', 'Bam Adebayo', 'Devin Booker',
      'Ja Morant', 'LeBron James', 'Kevin Durant', 'Kawhi Leonard',
    ]))],
  },
  ...
  'jun-2027-iem-cologne': {
    ...
    options: CS2_IEM_COLOGNE_TEAMS,
  },

  'jun-2027-love-island-launch': {
  ...
  'jun-2027-confederations': {
    ...
    options: FIFA_CONFEDERATIONS_NATIONS,
  },
  ...
  'jul-2027-wife-carrying': {
    ...
    options: ['Finland', 'Estonia', 'Latvia', 'Lithuania', 'Russia', 'USA', 'Germany', 'Other'],
  },

  'jul-2027-nathans-hotdogs': {
  ...
  'jul-2027-esports-world-cup': {
    ...
    options: [
      'Team Liquid', 'Cloud9', 'Fnatic', 'G2 Esports', 'Natus Vincere',
      'T1', 'Gen.G', 'Team Falcons', 'LOUD', 'Sentinels', 'Evil Geniuses',
    ],
  },
